...
//        option.setSeries(seriesList);
    ...
    @GetMapping("/ChangePage/{page}")
    @ResponseBody
    public PageTest<User> ChangePage(@PathVariable("page") Integer page){
        if (page==0){
            page=1;
        }

        PageTest<User> pageTest=new PageTest<>();

        PageHelper.startPage(page,3);
        List<User> list=userService.getAllUser();
        PageInfo<User> pageInfo=new PageInfo<>(list);
        pageTest.setPageInfo(pageInfo);
        pageTest.setList(list);

        return pageTest;
    }

    @GetMapping("/ChangePage")
    public String ChangePageBypageHelper(Model model,Integer page){
        if (page==0){
            page=1;
        }
        PageHelper.startPage(page,3);
